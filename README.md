...
아래 화면은 로컬 스택을 실제로 실행하고 `/api/v1/bootstrap`의 dbt·Airflow 상태를 불러온 뒤 캡처했습니다. 현재 개발 환경에는 원천 2개, 데이터 모델 5개, 파이프라인 2개, 품질 규칙 38개가 등록되어 있습니다. 파이프라인과 실행 이력은 로컬 메타스토어 상태에 따라 달라질 수 있습니다.

### 1. 운영 상태 확인

![홈 화면](docs/images/home.jpg)

홈에서는 최근 파이프라인 실행, 품질 규칙 통과율, 모델별 빌드 시간, 최근 사용한 모델을 한 화면에서 확인합니다. 값은 별도 대시보드용 집계가 아니라 Airflow·dbt 실행 결과를 다시 사용합니다.

### 2. 원천 데이터 수집

![데이터 수집 설정](docs/images/ingest.jpg)

HTTP API 또는 CSV·JSON Lines 파일을 미리보기한 뒤 `raw` 원천 테이블로 적재합니다. 수집 단계에서는 값을 문자열로 보존하고, 타입 변환과 정제는 dbt 모델에서 처리합니다.

### 3. 모델 정의와 컬럼 계보 확인

![데이터 모델과 컬럼 계보](docs/images/model.jpg)

모델 단위 관계와 컬럼 단위 흐름을 같은 캔버스에서 확인합니다. 컬럼 상세 보기를 켜면 복사된 컬럼과 `CASE`, 집계, 함수 등으로 계산된 변환 컬럼을 입력 컬럼까지 추적할 수 있습니다.

### 4. 실행 흐름 구성

![파이프라인 실행 의존성](docs/images/pipeline.jpg)

각 파이프라인 안의 태스크 순서는 dbt `ref()` 관계로 계산됩니다. 이 화면에서는 파이프라인 간 성공 후 실행 관계와 데이터 이벤트 트리거를 구성하고 실행 상태를 확인합니다.

### 5. 품질 결과 확인

![데이터 품질 대시보드](docs/images/quality.jpg)

dbt 테스트를 규칙으로 모아 전체 점수, 실패·주의 건수, 모델별 결과를 보여줍니다. 규칙 화면에서 추가·수정한 내용은 해당 모델의 YAML에 반영됩니다.

대표 사용자 흐름은 다음과 같습니다.

```text
API·파일 미리보기 → 원천 테이블 적재 → dbt 모델 정의 → 컬럼 계보 확인
        → 파이프라인 생성·실행 → 품질 결과 및 변경 이력 확인
```
...
관련 화면: [데이터 수집](docs/images/ingest.jpg)
...
관련 화면: [데이터 모델](docs/images/model.jpg)
...
관련 화면: [데이터 파이프라인](docs/images/pipeline.jpg)
...
관련 화면: [데이터 품질](docs/images/quality.jpg)
...
관련 화면: [홈](docs/images/home.jpg), [데이터 모델](docs/images/model.jpg)
