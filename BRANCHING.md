...
`feature/<변경내용>` — 무엇을 하는 브랜치인지 한 눈에 읽히게 쓴다.
...
- 손대는 층 이름(`ui` · `api` · `dbt` · `infra`)을 앞에 붙이면 목록이 정렬돼 편하지만
  **의무는 아니다.** 변경 내용이 이미 층을 말해 주는 경우가 많다.
- 한 브랜치가 세 층 이상에 걸치면 너무 크다는 신호다 — 갈라라.
- 이름은 목록에서 골라내기 위한 것이고, **무엇을 왜 했는지는 커밋 메시지가 맡는다.**
...
dbt 프로젝트는 이 저장소에 없다 (별도 저장소, `DBT_PROJECT_DIR` 로 가리킨다).
그쪽을 고쳤으면 그쪽 저장소에서 커밋하고, 여기서는 **콘솔이 그 변화를 읽는지**만 본다.

```bash
export DBT_PROJECT_DIR=<프로젝트 경로>
source ./env.sh
dbt build                                        # 기준 숫자는 프로젝트마다 다르다
curl -s -X POST localhost:8000/api/v1/reparse     # 콘솔에 반영
```
...
| 상태 | 위치 | 옮긴 뒤 |
| --- | --- | --- |
| dbt manifest | dbt 프로젝트의 `target/` (git 제외) | 다시 파싱해야 화면이 그 브랜치의 모델을 본다 — 서버 기동은 파싱하지 않는다 |
...
**제목은 액션으로 시작한다.** 목록을 훑을 때 무엇이 일어난 커밋인지가 먼저 보여야 한다.

```
<액션>: <무엇을 했는지 — 한국어, 수치가 있으면 넣는다>
...
| 액션 | 쓸 때 | 예 |
| --- | --- | --- |
| `add` | 없던 것을 만들었다 | `add: 계보 화면에 컬럼 검색` |
| `fix` | 고장난 것을 고쳤다 | `fix: 부팅 때 콘솔 에러 2건 — 로드 중 render() 호출` |
| `refactor` | **동작을 바꾸지 않고** 정리했다 | `refactor: pagePipeline 12겹 → 1` |
| `remove` | 지웠다 | `remove: 도달 불가한 옛 ERD 구현 21개 이름` |
| `docs` | 문서만 손댔다 | `docs: 브랜치 전략과 층별 완료 기준` |
| `chore` | 설정·도구·의존성 | `chore: gitignore 에 데모 녹화 산출물 추가` |

- **하나만 고른다.** 두 개를 붙이고 싶으면(`add`+`remove`) 커밋이 두 개여야 한다는 뜻이다.
  정말 한 덩어리라면 주된 쪽을 쓰고 본문에서 나머지를 설명한다.
- `refactor` 는 동작 불변일 때만 쓴다. 동작이 바뀌었으면 `add` 나 `fix` 다 —
  이 구분이 흐려지면 «refactor 커밋인데 화면이 달라졌다» 를 찾을 수 없게 된다.
- 범위를 괄호로 넣지 않는다(`add(ui):`). 필요하면 제목 문장에 자연스럽게 쓴다.
