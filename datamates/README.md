...
ui/js/b00~52.js 화면 코드 — 원래 index.html 의 인라인 블록을 순서 그대로 파일로 뗀 것
...
덮어쓰기 사슬로 남아 있다** — 남은 사슬 목록과 안전한 작업 절차는
[tools/ui-refactor/](../tools/ui-refactor/README.md) 참고.

같은 이름의 정의가 여럿 보이면 **실행 순서상 마지막(파일 번호가 큰 쪽, api.js 가 최후)**
이 이긴다. 수정 전에 `node tools/ui-refactor/chain.js <이름>` 으로 사슬부터 확인할 것.
