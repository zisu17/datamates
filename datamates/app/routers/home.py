...
독립적인 조회는 동시에 실행하고, 1일·7일처럼 범위가 겹치는 통계는 한 번의 스캔으로
계산한다. 개별 엔드포인트는 다른 화면에서도 사용하므로 유지한다.
"""
...
def _storage_safe() -> dict[str, Any]:
    ...
    거짓이 되고, 300초짜리 캐시를 매번 무시한 채 MinIO를 다시 훑는다.
    아래 catalog_volume · daily · runs도 같은 이유로 값을 명시한다.
    """
    try:
