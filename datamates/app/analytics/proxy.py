...
── 왜 경로를 바꾸지 않는가 ─────────────────────────────────────
처음 설계는 «/superset/* 로 받아 접두사를 벗겨 넘긴다» 였다. 두 가지를 시도했고
5.0.0 에서 둘 다 실패했다 — Superset 이 만드는 절대 경로에 접두사가 붙지 않는다.

    X-Forwarded-Prefix 헤더 → 자산 경로 27개 중 0개 적용
    SUPERSET_APP_ROOT 설정  → 같음 (5.0 에서 BETA, 알려진 버그)

접두사가 안 붙으면 브라우저가 /static/… 을 이 서버에 요청하고 404 를 받아
화면이 통째로 깨진다. HTML·JS 본문을 고쳐 쓰는 방법도 있지만 JS 번들이 수 MB 이고
런타임에 조립하는 주소는 잡지 못한다.

그래서 **경로를 그대로 중계한다.** 이 서버가 Superset 의 경로를 같은 이름으로
...
<style id="datamates-chrome">
  ...
  /* 좌측 필터 바는 **건드리지 않는다.**
     예전에는 위치 기반 선택자로 48px 아이콘 레일까지 접었다. 그때는 플랫폼이 차트를
     직접 그렸고 필터도 우리 것이라 엔진의 필터 바가 자리만 차지했기 때문이다.
     지금은 상세 화면이 엔진 대시보드를 그대로 띄우므로 그 필터 바가 곧 사용자가
     쓰는 필터다. 접으면 기간·전환율·자치구를 고를 데가 사라진다.

     겸사겸사 위험도 하나 없어졌다. 그 선택자는 nth-child 로 DOM 위치를 짚고 있어
     엔진이 레이아웃을 바꾸면 엉뚱한 칸을 회색으로 칠했다. */

  /* ── 로딩 Skeleton ────────────────────────────────────────────
     빈 카드 대신 자리를 유지한다. 그라디언트를 쓰지 않고 단색 블록 +
     불투명도만 움직인다(디자인 기준의 «그라디언트 배경 금지»).

     **전용 훅에만 건다.** 처음엔 `.loading` 도 함께 잡았는데, Superset 이 그 클래스를
     여러 곳에 쓰기 때문에 이미 렌더된 차트 영역까지 회색 블록으로 덮었다.
     data-test 훅은 로딩 표시 전용이라 오작동하지 않는다. */
