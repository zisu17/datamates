...
# ─────────────────────────────────────────────────────────────
# 임베드 — 대시보드를 iframe 에 넣기 위한 두 조각
# ─────────────────────────────────────────────────────────────
#
# 일반 대시보드 경로를 iframe 에 넣으면 셸만 뜨고 차트가 렌더되지 않는다(P2 확인).
# 임베드용 경로는 /embedded/<uuid> 이고 두 가지가 필요하다.
#
#   1. 대시보드를 embeddable 로 등록해 uuid 를 받는다 (한 번)
#   2. 게스트 토큰을 발급한다 (5분마다, SDK 가 자동 갱신 요청)
#
# 게스트 토큰은 **반드시 서버가 발급한다.** 브라우저가 직접 발급하려면 서비스 계정
# 자격증명이 브라우저로 나가야 한다. 권한 모델이 없다고 해서 자격증명을 내보낼
# 이유는 없다 — 발급 창구를 서버에 두는 것이 비용도 들지 않는다.
...
def place_on_dashboard(dash_id: int, chart_id: int, name: str,
    ...
    position_json 은 Superset 이 대시보드 배치를 담는 트리다. 차트를 관계로만
    연결하면(dashboards 필드) 목록에는 잡히지만 화면에 그려지지 않는다 —
    P2 에서 확인했다. 그래서 배치까지 해 준다.
