/* ── b57 — 데이터 수집 개편 (커넥터) ─────────────────────────────────────
   ============================================================
   claude.ai/design 프로젝트 «데이터 수집 커넥터» (데이터 수집 커넥터.dc.html)를
   옮긴다. api.js 의 v6.1 데이터 수집 화면을 갈아끼우는 층이다 —
   b56 이 파이프라인 화면에 한 것과 같은 방식으로, 화면 함수만 다시 정의한다.

   ── v6.1 → 커넥터 로 달라진 것 ───────────────────────────────
   ① 이름 — «수집 작업» 이 «커넥터» 가 된다. 작업 은 «언제 도는가» 를 떠올리게
      하는데, 이 화면이 정하는 것은 «어디에 붙어 무엇을 어디로 넣는가» 뿐이다.
      실행 주기와 실행 상태는 데이터 파이프라인이 맡는다. 목록 위 안내 줄이
      그 경계를 한 문장으로 말하고, 파이프라인으로 건너가는 길을 둔다.
   ② 좌측 사이드바 — **없앴다.** 데이터 모델·파이프라인은 항목끼리 관계가 있어
      («이 모델의 원천은 무엇인가») 목록을 늘 곁에 두고 오가야 하지만, 커넥터끼리는
      아무 관계가 없다. 하나를 보는 동안 다른 것을 함께 봐야 할 일이 없는데
      화면 폭의 1/6 을 상시로 내주고 있었고, 사이드바와 「커넥터 목록」 탭에
      검색칸이 하나씩 생겨 둘 중 어느 것이 지금 거른 결과인지도 알 수 없었다.
      목록은 「커넥터 목록」 탭 한 곳에만 두고, 열어 둔 커넥터는 탭 줄이 기억한다.
   ③ 목록 — 한 덩어리 격자 → 유형별 구획 + 검색 + 유형·상태 거르기.
      카드는 머리(이름·적재 대상·상태)·본문(적재 방식·연결·최근 실행)·
      바닥(쓰는 데이터 모델·수정 시각) 세 층으로 나뉜다.
   ④ 상세 — 한 줄 세로 나열 → 2단. 왼쪽은 «이 커넥터가 무엇인가»(연결 정보 ·
      적재 설정 · 적재 컬럼 · 수집 데이터), 오른쪽은 «그래서 어디에 쓰이는가»
      (만들어진 SOURCE · 쓰는 데이터 모델 · 연결된 파이프라인).
   ⑤ 만들기·수정 — 모달 한 장 → **4단계 위저드 탭**. 유형 선택 → 연결 정보 →
      적재 설정 → 확인. 모달은 스크롤이 길어 «지금 어디까지 정했는지» 가
      보이지 않았고, 연결 테스트 결과와 입력이 같은 세로줄에서 겹쳐 있었다.
      위저드는 연결 테스트를 오른쪽 레일에 고정해 입력과 나란히 둔다.
   ⑥ 요청 파라미터 — 새로 생겼다. 수집 엔진은 예전부터 config.params 를 읽어
      («_fill_placeholders» 로 날짜 자리표시자까지 채워서) 요청에 붙였는데,
      화면에 그것을 넣을 칸이 없어 주소창에 직접 적는 수밖에 없었다. 인증키를
      주소에 적으면 마스킹이 걸리지 않는다 — 그래서 비밀 표시(secret_params)도
      함께 둔다.

   ── 마크업 규칙 ──────────────────────────────────────────────
   b56(파이프라인)·b14(품질)과 같다. 레이아웃 골격은 앱 어휘(.mod · .mod-l ·
   .page)를 그대로 쓰고, 잎사귀 부품만 DS 컴포넌트 층(.wc-card · .wc-badge ·
   .wc-btn · .wc-table · .wc-list)을 쓴다. 아이콘은 원본의 Tabler CDN 마스크
   대신 앱 스프라이트(#i-*, ic/ic14)다 — 한 화면에 아이콘 소스가 두 벌이 되고
   오프라인에서 아이콘만 사라지는 것을 막는다.

   ── 지어내지 않은 자리 ───────────────────────────────────────
   원본에는 있으나 서버에 근거가 없어 옮기지 않은 것들이다. 빈 칸으로 두느니
   그 자리에 실제로 아는 사실을 넣었다.
   · «버전 이력 v4 · v3 · v2 · v1» — 커넥터에는 버전이 없다. store 의
     ingest_job 은 현재 상태 한 벌만 들고 created_at · updated_at 만 남긴다
     (모델과 달리 model_history 같은 이력 표가 없다). 만든 시각·고친 시각만 적는다.
   · «인증 · 2026.12.31 만료» — 자격증명에 만료일을 저장하는 자리가 없다.
     인증 방식(Bearer · 헤더 키 · 쿼리 파라미터)과 «저장됨» 여부만 적는다.
   · «연결 확인 2026.08.13 오후 3:00» — 확인 시각을 남기는 표가 없다. 대신
     연결 확인 을 **지금 누르면** 실제로 원천을 두드리고(/ingest/preview),
     그 결과를 이 세션 동안 기억한다. 기억한 값에는 «이번 세션» 이라고 적는다.
   · «파이프라인 3개에서 사용» — 커넥터를 파이프라인이 직접 참조하지는 않는다.
     실제 사슬은 커넥터 → SOURCE → 데이터 모델 → 파이프라인 이라, 그 사슬을
     따라가 «쓰는 데이터 모델» 과 «그 모델을 만드는 파이프라인» 을 각각 센다.
   · «원본 파일 region_code_2026.csv · 1.2MB» — 파일 커넥터는 올린 파일을
     보관하지 않는다(그 자리에서 적재하고 버린다). 파일 형식·구분자만 적는다.
   ============================================================ */
(function () {
  ...
  /* 유형. 원본의 «REST API · 파일 업로드» 를 그대로 쓴다 — 예전 이름
     («API 연동 · 파일 올리기») 은 동사라 목록의 구획 제목으로 읽히지 않았다. */
  const CK = {
  ...
  async function cnCheck(j) {
    ...
    try {
      ...
      /* 행이 0건인 것과 연결이 안 되는 것은 다른 일이다.
         서버가 **실제 실행과 같은 조건**(구간·지역)으로 물어보므로, 0건은 대개
         «그 조건에 원래 자료가 없다» 는 뜻이다 — 분양권 전매처럼 드문 거래는
         종로구 한 달에 0건인 게 정상이다. 그걸 경고로 적으면 멀쩡한 커넥터를
         고치려 들게 된다. 다만 행이 없으면 레코드 경로까지는 확인하지 못한 것이라,
         그 사실을 감추지 않고 함께 적는다. */
  ...
  ingDetailView = function (j) {
    ...
    /* 인증 한 줄. 자격증명을 가리키면 그 이름과 만료일이 진짜 정보이고,
       예전 방식(값을 커넥터 안에 직접 둔 것)이면 «직접 입력» 이라고 밝힌다 —
       둘을 같은 모양으로 그리면 어느 커넥터가 옮겨졌는지 알 수 없다. */
  ...
  async function wizRunPrev(force) {
    ...
    try {
      ...
      /* **실패는 이미 아는 것을 지우지 않는다.**

         예전에는 여기서 st.cols 를 비웠다. 그러면 수정 화면에서 한 번 실패하는
         순간(키 만료·자격증명 오선택·네트워크 끊김) 저장돼 있던 컬럼이 사라지고,
         «컬럼을 먼저 확인해야 합니다» 에 막혀 되돌리지도 저장하지도 못한다 —
         방금 바꾼 설정을 원래대로 되돌리려 해도 저장 자체가 막힌다.

         컬럼을 갈아 끼우는 것은 성공한 조회만 할 수 있는 일이다. 표본 행은
         지운다 — 그건 이번 응답의 것이라 남겨 두면 실패 화면에 옛 값이 남는다. */
  ...
  function wizConnStep(main) {
    ...
    if (st.kind === 'api') {
      ...
      /* 요청 파라미터 — 수집 엔진이 예전부터 읽던 config.params 를 처음으로 화면에 연다 */
  ...
  /* 바깥에서 부르던 이름을 위저드로 돌린다 — 예전 모달을 여는 곳이 남아 있어도 동작한다 */
