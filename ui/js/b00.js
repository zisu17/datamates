...
const PIPES = [
  { id:'pl_exam', name:'일별 검사 현황 생성', targets:['stg_examination_result','fct_patient_examination','agg_daily_examination'],
    ...
    /* 서버 응답이 오면 덮어쓰이는 폴백 값이다. 특정 도메인의 컬럼·테이블 이름을
       쓰지 않는다 — 서버가 죽었을 때만 보이는 화면인데, 거기에 이 제품이 다루지
       않는 데이터의 이름이 뜨면 사용자가 원인을 엉뚱한 곳에서 찾는다. */
    err:{ step:'데이터 검증', title:'집계 데이터 생성에 실패했습니다.',
      body:'필수값이 비어 있는 데이터와 기준정보에 없는 값이 확인되었습니다. 실패한 단계와 오류 내용을 확인한 후 다시 실행할 수 있습니다.',
      log:['05:19:02  데이터 검증 시작 (검증 8건)','05:19:11  필수값 누락 — 12건 실패','05:19:14  참조 무결성 오류 — 기준정보 미존재 5건 실패','05:19:18  검증 실패로 이후 단계 중단','05:19:18  완료 — 성공 6 · 실패 2 · 중단 1'] } },
